...
8. After user confirms: merge the PR via squash. Compose the commit message — title: `<PR title> (#<number>)`. Do not use GitHub's default squash message. Compose the body using this fallback chain: use the proposal content if available; if the proposal is missing or unavailable, fall back to the PR body; if that is empty, use the title only. When proposal content is available, format the body as: Why text, blank line, What Changes bullets, then issue-closing references (e.g., `Closes #N`). Do not duplicate issue-closing references if they already appear in the Why section. Set proposal status: completed. Post-merge: clean up worktree if applicable.

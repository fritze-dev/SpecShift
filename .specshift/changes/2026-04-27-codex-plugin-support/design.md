...
**Bootstrap behavior (third-pass reversal)**: Fresh `specshift init` generates both `AGENTS.md` (full body, agnostic SSOT) and `CLAUDE.md` (one-line `@AGENTS.md` import stub). The stub is a pointer, not a content duplicate, so SSOT is preserved. Existing files are never overwritten on re-init; standard-sections checks remain passive WARNING-only. (The second-pass narrowing to AGENTS-only was reversed before merge — see proposal §"Scope Reversal".)

**Spec deltas**:

- `docs/specs/multi-target-distribution.md` — manifests-at-root requirement, agnostic-skill-body requirement, bootstrap-pattern wording (fresh init generates both AGENTS.md and the CLAUDE.md `@AGENTS.md` import stub), version bump 1 → 3.
...
- `docs/specs/project-init.md` — `${CLAUDE_PLUGIN_ROOT}/templates/...` → "the plugin's `templates/` directory" prose; Bootstrap Files Generation requirement: fresh init writes both `AGENTS.md` (full body) and `CLAUDE.md` (one-line `@AGENTS.md` import stub); existing files never overwritten on re-init; `Purpose` updated. Version bump 6 → 8.
...
| Fresh init writes both AGENTS.md and CLAUDE.md (one-line `@AGENTS.md` stub) — third-pass reversal of the second-pass AGENTS-only narrowing | Zero-cost symmetry: the stub is a pointer (not a content duplicate), so SSOT is preserved. Codex ignores `CLAUDE.md`; Claude Code uses it to expand `@AGENTS.md` at session start, activating the documented memory-import pattern without manual copy. The narrowing left Claude Code consumers with silently-inactive memory imports — a maintainer-zentriert "narrower bootstrap" that did not benefit users. | (A) Fresh-init AGENTS-only with manual stub copy — the rejected second-pass narrowing; re-introduces stille Nicht-Funktion under Claude Code. (C) Auto-detect environment and pick which file to write — env detection remains out of scope per ADR-003. |
...
- **Existing CLAUDE.md projects expect auto-generation.** A consumer that previously ran `specshift init` on a stale CLAUDE.md and expected it to be regenerated will now see a WARNING but no rewrite. **Mitigation:** documented in the migration scenario; consumers can manually delete CLAUDE.md to opt back in to a clean re-bootstrap (which writes both `AGENTS.md` and the `@AGENTS.md` import stub).
...
6. **Update `src/templates/workflow.md` `## Action: init` instruction** — fresh init generates both `AGENTS.md` (from `templates/agents.md`) and `CLAUDE.md` (one-line `@AGENTS.md` import stub from `templates/claude.md`); existing files never overwritten on re-init; standard-sections checks remain passive WARNING-only.

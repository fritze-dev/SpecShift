...
The Codex install path documented in the spec (`codex plugin marketplace add fritze-dev/SpecShift`) does not surface the plugin in practice — a user reported the failure on PR #46's merged distribution. This falsifies the "Codex single-plugin auto-discovery" assumption baked into `multi-target-distribution.md` and ADR-003 and leaves Codex consumers unable to install. Issue #51's acceptance branch (b) is the response: ship the documented `.agents/plugins/marketplace.json` catalog file, add the generated `plugins/specshift/` payload that Codex can resolve from `/plugins`, flip the spec and ADR to mandate it, and correct the README/AGENTS.md narrative.

## What Changes

- **NEW** `.agents/plugins/marketplace.json` at the repository root, hand-edited, populated with the Codex-documented schema (`name`, `interface.displayName`, `plugins[].source: { source: "local", path: "./plugins/specshift" }`, `plugins[].policy.installation: "AVAILABLE"`, `plugins[].policy.authentication: "ON_INSTALL"`, `plugins[].category: "Coding"`).
- **NEW** `plugins/specshift/` generated Codex marketplace payload containing `.codex-plugin/plugin.json` and the compiled `skills/specshift/` tree.
...
- **MODIFIED** `README.md` — replace the wrong `codex /plugins` line with `codex plugin marketplace add fritze-dev/SpecShift` plus a `/plugins` install/enable instruction. Add a Codex "Update" subsection using `codex plugin marketplace upgrade specshift`.
...
- **Distribution**: NEW root file `.agents/plugins/marketplace.json` plus generated `plugins/specshift/` payload. Codex consumers that previously failed to install will now succeed via `codex plugin marketplace add fritze-dev/SpecShift` because the catalog file points at a non-empty plugin root that `/plugins` can resolve.

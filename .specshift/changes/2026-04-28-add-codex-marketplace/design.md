...
The user-observed install failure on `codex plugin marketplace add github:fritze-dev/specshift` falsifies the auto-discovery assumption underlying PR #46's distribution layout. The spec already describes the fallback (`.agents/plugins/marketplace.json` with the documented Codex schema) as a conditional. This design realises that fallback as a concrete file plus the necessary changes to keep build automation, CI, spec, ADR, README, AGENTS.md, and constitution self-consistent.
...
- `.agents/plugins/marketplace.json` — Codex marketplace catalog. Top-level: `name: "specshift"`, `interface.displayName: "SpecShift"`. `plugins` array with one entry: `name: "specshift"`, `description`, `source: { source: "local", path: "../../.codex-plugin" }`, `policy: { installation: "user-required", authentication: "none" }` (documented baseline values), `category: "Coding"`. No `version` field on the entry — version is sourced from `.codex-plugin/plugin.json`.
...
- `README.md` — replace the wrong `codex /plugins` line with `codex plugin marketplace add github:fritze-dev/specshift` + `codex plugin install specshift`. Add Codex Update subsection mirroring Claude's: `codex plugin marketplace update specshift && codex plugin update specshift`.
...
| Catalog entry uses `source: { source: "local", path: "../../.codex-plugin" }` | Documented Codex schema form; "local" + relative path is the documented form for repo-internal entries | `source: "../../.codex-plugin"` bare-string form (Claude marketplace's form, not Codex's) |
...
- **Catalog `path` resolves wrong on consumer machines** → Mitigation: use `../../.codex-plugin` relative path, which resolves identically on every clone of the repo because both files live at known repo-root locations. Verified by repo-root inspection during audit.
...
No consumer-side migration required. Existing Claude Code installs are unaffected. New Codex installs that previously failed will now succeed by re-running `codex plugin marketplace add github:fritze-dev/specshift`. Codex consumers who had an aborted/partial install from the previous release should re-run the add command after this change is merged.
...
- The `policy.installation` baseline value `"user-required"` and `policy.authentication` baseline value `"none"` reflect the documented defaults for a public open-source plugin. <!-- ASSUMPTION: Codex policy defaults -->
- Relative paths in `plugins[].source.path` are resolved relative to the catalog file's location (i.e. `.agents/plugins/`), not relative to the repository root, per the documented Codex schema. <!-- ASSUMPTION: Codex relative-path resolution -->
- A re-run of `codex plugin marketplace add github:fritze-dev/specshift` after this change lands on `main` will succeed for the user who reported the original failure. <!-- ASSUMPTION: Catalog file unblocks install -->

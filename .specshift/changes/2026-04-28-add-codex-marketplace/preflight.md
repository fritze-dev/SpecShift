...
| 2 | The `policy.installation` baseline `user-required` and `policy.authentication` baseline `none` reflect documented defaults for a public open-source plugin | design.md | Yes | Acceptable Risk — these values are conservative defaults. If the documented defaults differ in a way that affects install, the catalog file is hand-edited and trivially adjustable. |
| 3 | Relative paths in `plugins[].source.path` are resolved relative to the catalog file's location | design.md | Yes | Acceptable Risk — the documented schema specifies `path` as relative; using `../../.codex-plugin` from `.agents/plugins/marketplace.json` resolves to the repo root's `.codex-plugin/` on every clone. |
| 4 | A re-run of `codex plugin marketplace add github:fritze-dev/specshift` after this change lands on `main` will succeed | design.md | Yes | Acceptable Risk — this is the working hypothesis for the change. If the re-run still fails, the next change is to investigate Codex CLI version differences (out of this change's scope). |

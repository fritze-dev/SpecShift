...
- [ ] 1.1. Update `src/templates/workflow.md`: bump `template-version` 4→5, add `release` to `actions` array, add `release:` config block to frontmatter, add `## Action: release` section with tool-agnostic instruction
- [ ] 1.2. Update `.specshift/WORKFLOW.md`: add `release` to `actions` array, add `release:` config block (`request_review: copilot`), add `## Action: release` section with project-specific instruction

## 2. Implementation

- [ ] 2.1. Update `src/skills/specshift/SKILL.md`: add conditional `finalize → release` auto-dispatch in the `### finalize` dispatch section (condition: `auto_approve: true` AND `release` in `actions` array)
- [ ] 2.2. Run `bash scripts/compile-skills.sh` to regenerate `.claude/skills/specshift/` from updated source files

## 3. QA Loop & Human Approval

- [ ] 3.1. Metric Check:
  - [ ] `specshift release` correctly identifies PR state on invocation — PASS / FAIL
  - [ ] Auto-dispatch chain finalize→release when auto_approve and release in actions — PASS / FAIL
  - [ ] Backward-compatible: consumers without release in actions see no change — PASS / FAIL
  - [ ] `bash scripts/compile-skills.sh` succeeds — PASS / FAIL

...
- After `bash scripts/compile-skills.sh` runs in finalize, `skills/specshift/` contains **0** `worktree` mentions. The earlier round-1 fix had narrowed the grep invariant to allow legacy-frontmatter documentation that propagated through compile; a follow-up cleanup (this round) replaced that field-specific spec text with a generic "skills ignore unknown frontmatter fields" rule and dropped the legacy edge case, so the spec no longer mentions the removed field. The narrowed grep is now equivalent to the broad grep — both return 0.

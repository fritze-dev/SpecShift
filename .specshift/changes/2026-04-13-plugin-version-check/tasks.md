...
- [x] 1.1. Update `src/templates/workflow.md`: add `plugin-version: ""` to frontmatter, bump `template-version` from 2 to 3

## 2. Implementation

- [x] 2.1. [P] Restructure `src/skills/specshift/SKILL.md` — rewrite Steps 1-5:
...
- [x] 2.2. [P] Update `.specshift/WORKFLOW.md` (project instance): add `plugin-version: 0.1.3-beta`, bump `template-version` to 3
- [x] 2.3. Sync `src/templates/workflow.md` body sections to `.specshift/WORKFLOW.md` — ensure Action instructions match (respecting intentional project-specific overrides per constitution)

## 3. QA Loop & Human Approval

- [x] 3.1. Metric Check:
  - [x] SKILL.md reads WORKFLOW.md exactly once (Step 1) — PASS
  - [x] Version mismatch produces advisory warning with both versions — PASS
  - [x] Version match produces no output — PASS
  - [x] Missing `plugin-version` field produces note — PASS
  - [x] `init` action skips version check — PASS
  - [x] Template `template-version` bumped to 3 — PASS

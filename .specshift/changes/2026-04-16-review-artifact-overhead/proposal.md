...
- **Tasks template updated**: `requires: [preflight]` (was `[tests]`). Gains apply-phase test guidance for projects with configured test frameworks. Template-version 5 → 6 (PR #60 already bumped to 5; semantic-heading discipline preserved).
...
- `quality-gates`: Audit verification dimensions reworded — Testing dimension references "specs (direct scenario verification)" instead of tests.md. Positional task-step references (`step 3.2`, `step 3.5`) replaced with semantic anchors per PR #60 heading discipline.

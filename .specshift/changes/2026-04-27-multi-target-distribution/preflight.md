...
| Codex user wants discovery via `/plugins` | Codex Discovery via Marketplace Add | Codex install discovers the plugin manifest; No Codex marketplace catalog file shipped | `.codex-plugin/plugin.json` (auto-discovered); spec scenario verifies absence of `.agents/plugins/marketplace.json` |

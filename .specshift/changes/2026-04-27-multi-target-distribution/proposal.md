...
- **Codex install via auto-discovery**: Codex consumers install via `codex plugin marketplace add github:fritze-dev/specshift`, which reads `.codex-plugin/plugin.json` at the repo root directly. No `.agents/plugins/marketplace.json` catalog file is shipped (single-plugin auto-discovery is the documented Codex path; an earlier iteration of this change shipped the catalog file with a wrong, Claude-style schema and was reverted after Shopify-AI-Toolkit abgleich confirmed the auto-discovery path).
- **Move plugin manifests out of `src/` to the repo root.** `.claude-plugin/plugin.json`, `.claude-plugin/marketplace.json`, `.codex-plugin/plugin.json` are hand-edited per-target metadata and live exclusively at the root. The `src/.claude-plugin/` indirection is removed.
...
- **Introduce `src/VERSION` as the single version source of truth.** Plain-text file, one line, SemVer. The compile script reads it and stamps the version into all three root manifest/marketplace files.
- **Extend `scripts/compile-skills.sh`** to: (1) read the version from `src/VERSION` (not a manifest), validate it as SemVer 2.0, (2) stamp the three root files (`.claude-plugin/plugin.json`, `.claude-plugin/marketplace.json`, `.codex-plugin/plugin.json`) symmetrically via `jq` while preserving all non-version fields and values (JSON formatting may be normalized by `jq`), (3) cross-check that each emitted version equals the SoT post-stamp (fail the build on drift), (4) emit one shared agnostic skill tree to `./skills/specshift/`, (5) clean up legacy compiled outputs (`.claude/skills/specshift/`, `.claude/.claude-plugin/`).
...
- **Build script (`scripts/compile-skills.sh`):** rewritten version-handling — read from `src/VERSION`, symmetric jq-stamping into all three root files, post-stamp cross-check; path migration `.claude/skills/` → `./skills/`; jq becomes a hard preflight requirement.

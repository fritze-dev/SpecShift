...
└── plugin.json                # version stamped, interface block hand-edited
                                # Codex consumers install via
                                # `codex plugin marketplace add github:owner/repo`
                                # which auto-discovers this manifest — no
                                # .agents/plugins/marketplace.json shipped
...
The version-bump step changes from "edit `src/.claude-plugin/plugin.json` and sync `.claude-plugin/marketplace.json`" to "edit `src/VERSION` only". The subsequent compile run propagates the new version into all three root files.
...
- **G2 — Symmetric versions**: All three root manifest/marketplace files declare the same version after `bash scripts/compile-skills.sh`. PASS/FAIL: `jq -r` over the four version locations returns four equal values, all matching `cat src/VERSION`.
- **G3 — Agnostic SoT for version**: `src/VERSION` is the only file the maintainer edits to bump the version. PASS/FAIL: `git log --oneline -1 -- src/VERSION` shows the bump commit; the three root files were modified in the same commit by the compile script, not by hand.
...
| Compile script stamps version into all three root files and cross-checks each post-stamp | Eliminates silent drift; previously the Claude marketplace version was hand-edited and unchecked | Stamp only the manifests; trust hand-edits on marketplaces |
...
| Do not ship `.agents/plugins/marketplace.json` (Codex consumers install via auto-discovery) | Shopify-AI-Toolkit reference repo does not ship one; OpenAI Codex docs document auto-discovery for single-plugin repos; an earlier iteration shipped a catalog file with a wrong, Claude-style schema and was reverted after Shopify abgleich | Ship the file with the correct Codex schema (`name`, `interface.displayName`, `plugins[].source: {source, path}`, `plugins[].policy`); ship with Claude-style schema (rejected — wrong format); ship a stub (rejected — adds confusion) |
...
4. Create `.codex-plugin/plugin.json` at the root with the full Codex schema. (Codex consumers install via `codex plugin marketplace add github:owner/repo` which auto-discovers this manifest; no `.agents/plugins/marketplace.json` is shipped.)
...
11. Run `bash scripts/compile-skills.sh` — emits the shared `./skills/specshift/`, stamps all three root files, validates.
...
- The Codex CLI's plugin manifest schema (`.codex-plugin/plugin.json`) and skill discovery paths described in OpenAI's `developers.openai.com/codex` documentation are stable as of 2026-04-28. The `codex plugin marketplace add github:owner/repo` auto-discovery path for single-plugin repos is the supported install method. <!-- ASSUMPTION: Codex CLI plugin schema and auto-discovery stable -->

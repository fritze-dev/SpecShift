...
- **Review-pending gate:** If a review was requested (via `request_review` config) but no review decision has been submitted yet, report "Review pending — waiting for reviewer decision" and suggest re-running the specshift skill with `review` later. Do NOT offer merge.

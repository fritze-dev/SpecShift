...
- Plugin manifests AND marketplace files live hand-edited at the repo root: `.claude-plugin/plugin.json` (Claude manifest, version source of truth), `.claude-plugin/marketplace.json` (Claude marketplace), `.codex-plugin/plugin.json` (Codex manifest), and `.agents/plugins/marketplace.json` (Codex marketplace). None of these are generated from `src/` — the compile script reads the version from `.claude-plugin/plugin.json` and stamps it into the Codex manifest and the Codex marketplace via `jq`, preserving every other field verbatim.
...
- **Post-apply version bump:** During the post-apply workflow, automatically increment the patch version in `.claude-plugin/plugin.json` (e.g., `1.0.3` → `1.0.4`) and sync the `version` field in `.claude-plugin/marketplace.json` to match. The Claude manifest at the repo root is the version source of truth — the compile script stamps this version into the Codex manifest (`.codex-plugin/plugin.json`) and the Codex marketplace (`.agents/plugins/marketplace.json`) automatically, so they do not need manual edits. Display the new version. A GitHub Action automatically creates a git tag and GitHub Release when the version change is pushed to `main`. For intentional minor/major releases, manually set the version in `.claude-plugin/plugin.json` and `.claude-plugin/marketplace.json`, run `bash scripts/compile-skills.sh` to stamp the Codex manifest and marketplace, and push — the Action handles tagging and release creation.
- **Plugin source layout:** Plugin source code (skills, templates, action manifests) lives in `src/`. Plugin manifests and marketplace files are hand-edited at the repo root: `.claude-plugin/plugin.json`, `.claude-plugin/marketplace.json`, `.codex-plugin/plugin.json`, `.agents/plugins/marketplace.json`. The shared release directory at `./skills/specshift/` is built from `src/` via `bash scripts/compile-skills.sh` and contains the compiled skill, templates, and pre-extracted action files. The compile script does NOT generate manifests or marketplace files — it only stamps the Claude `plugin.json` version into the Codex manifest and Codex marketplace via `jq`. Project files (docs, CI, specs, changelog) stay at the repo root. The Claude `marketplace.json` uses `source: "./"`.
...
- **Agent instructions:** Project-level agent instructions live in `AGENTS.md` (agnostic single source of truth, full body, read by Codex natively and by Claude Code via the `@AGENTS.md` import expanded from `CLAUDE.md`). On fresh init, `specshift init` generates both `AGENTS.md` (from `templates/agents.md`) and a one-line `CLAUDE.md` import stub (from `templates/claude.md`) so Claude Code's documented memory-import pattern is active immediately. The stub is a pointer, not a content duplicate — normative rules live only in `AGENTS.md`. Instructions use tool-agnostic language.
- **Tool-agnostic instructions:** Specs, skills, and templates MUST describe intent (e.g., "create a draft PR") rather than hardcoding specific CLI tools (e.g., `gh pr create`). The plugin runs across environments with different tooling — agent web/desktop interfaces (MCP tools), terminal (gh CLI), or API-only. Concrete tool names may appear in parenthetical examples (e.g., "available GitHub tooling (gh CLI, MCP tools, or API)") but MUST NOT be the sole instruction. Compiled-into-skill text MUST avoid runtime-specific tokens such as `${CLAUDE_PLUGIN_ROOT}`; reference plugin-bundled assets in prose ("the plugin's `templates/` directory") so the same compiled skill works under any host plugin runtime.

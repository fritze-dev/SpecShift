...
- **Per-target manifests at the repo root:** `.claude-plugin/plugin.json`, `.claude-plugin/marketplace.json`, and `.codex-plugin/plugin.json` are hand-edited at the repository root. They carry per-target metadata; their `version` field is the only field automation touches (stamped from `src/VERSION` by the compile script). Codex consumers install via `codex plugin marketplace add github:owner/repo` which auto-discovers `.codex-plugin/plugin.json` — no separate Codex marketplace catalog file is shipped.

...
- Plugin manifests live hand-edited at the repo root: `.claude-plugin/plugin.json` (Claude manifest, version source of truth), `.claude-plugin/marketplace.json` (Claude marketplace), and `.codex-plugin/plugin.json` (Codex manifest). Manifests are NOT generated from `src/` — the compile script reads the version from `.claude-plugin/plugin.json` and stamps it into the Codex manifest and the Codex marketplace entry. The Codex marketplace entry lives at `.agents/plugins/marketplace.json` and is generated from `src/marketplace/codex.json`.
...
- **Plugin source layout:** Plugin source code (skills, templates, action manifests) lives in `src/`. Plugin manifests are hand-edited at the repo root: `.claude-plugin/plugin.json`, `.claude-plugin/marketplace.json`, `.codex-plugin/plugin.json`. The Codex marketplace template lives at `src/marketplace/codex.json`. The shared release directory at `./skills/specshift/` is built from `src/` via `bash scripts/compile-skills.sh` and contains the compiled skill, templates, and pre-extracted action files. The Codex marketplace entry (`.agents/plugins/marketplace.json`) is generated from `src/marketplace/codex.json`. Project files (docs, CI, specs, changelog) stay at the repo root. The Claude `marketplace.json` uses `source: "./"`.

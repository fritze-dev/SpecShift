...
- **Agent instructions:** Project-level agent instructions live in `AGENTS.md` (agnostic single source of truth, read by Codex natively and by Claude Code via the `@AGENTS.md` import in a hand-maintained `CLAUDE.md`). `specshift init` does NOT auto-generate `CLAUDE.md`; the plugin ships `templates/claude.md` as a one-line `@AGENTS.md` import stub for users who explicitly want Claude Code's documented memory pattern. Instructions use tool-agnostic language.

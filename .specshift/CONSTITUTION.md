...
- **Platform:** Claude Code and Codex plugin systems
...
- Claude marketplace metadata lives in `.claude-plugin/`; Codex plugin metadata lives in `.codex-plugin/`.
...
- **Release artifacts:** `.claude/` is the generated Claude release; `.codex-plugin/` and `skills/specshift/` are the generated Codex root-plugin release. They are committed to Git and MUST NOT be hand-edited — regenerate via `bash scripts/compile-skills.sh`.
...
- **Post-apply version bump:** During the post-apply workflow, automatically increment the patch version in `src/.claude-plugin/plugin.json` (e.g., `1.0.3` → `1.0.4`) and sync the `version` field in `src/.codex-plugin/plugin.json` and `.claude-plugin/marketplace.json` to match. The root Codex manifest carries the Codex plugin version; no separate Codex marketplace version is maintained. If versions are out of sync, use `src/.claude-plugin/plugin.json` as source of truth. Display the new version. A GitHub Action automatically creates a git tag and GitHub Release when the version change is pushed to `main`. For intentional minor/major releases, manually set the version in both plugin manifests and the Claude marketplace file, then push — the Action handles tagging and release creation.
- **Plugin source layout:** Plugin source code lives in `src/` (skills, templates, plugin manifests). The Claude release at `.claude/` and the Codex root-plugin release at `.codex-plugin/` plus `skills/specshift/` are built from `src/` via `bash scripts/compile-skills.sh` and contain compiled skills, templates, manifests, and pre-extracted action files. Project files (docs, CI, specs, changelog) stay at the repo root. The Claude marketplace file uses `source: "./.claude"`; Codex discovers the repository root through `.codex-plugin/plugin.json`, following the Shopify AI Toolkit layout.
...
- **AOT compilation:** After editing specs, run `bash scripts/compile-skills.sh` to regenerate both release directories. The finalize action runs this automatically.
- **Local development:** Claude developers register the local repo as marketplace via `claude plugin marketplace add <local-path> --scope user`; skill changes reload via `/reload-plugins`; version changes require `claude plugin update specshift@specshift`. Codex developers install or update the local checkout from the Codex `/plugins` UI.
...
- **Agent instructions:** Project-level Claude instructions live in `CLAUDE.md`; Codex bootstrap instructions generate `AGENTS.md`. Instructions use tool-agnostic language.
- **Tool-agnostic instructions:** Specs, skills, and templates MUST describe intent (e.g., "create a draft PR") rather than hardcoding specific CLI tools (e.g., `gh pr create`). The plugin runs across environments with different tooling — Claude Code Web, Codex, desktop CLI, MCP tools, or API-only. Concrete tool names may appear in parenthetical examples (e.g., "available GitHub tooling (gh CLI, MCP tools, or API)") but MUST NOT be the sole instruction.
...
### Post-Merge
- Update plugin locally (`claude plugin marketplace update specshift && claude plugin update specshift@specshift`, or update/reinstall SpecShift from Codex `/plugins`) — applies when change modifies files under `src/`, `.claude/`, `.codex-plugin/`, or `skills/specshift/`

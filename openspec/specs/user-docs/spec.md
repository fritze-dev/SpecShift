...
- **Initial-spec research.md too thin**: If the initial-spec archive's research.md lacks useful approaches or decisions for a capability, the agent SHALL attempt to derive Rationale from spec requirements and scenarios. Only omit Rationale if truly no design reasoning is derivable from the spec itself.

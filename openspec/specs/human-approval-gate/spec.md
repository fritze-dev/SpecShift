...
#### Scenario: Reject archiving without approval
...
<!-- ASSUMPTION: Spec updates during the fix loop do not require re-running the full artifact pipeline (preflight is not re-triggered automatically) -->
...
<!-- ASSUMPTION: The human reviewer is the same person who initiated the change or has sufficient context to approve -->
<!-- ASSUMPTION: "Approved" is the canonical approval token; the system recognizes it case-insensitively -->
<!-- ASSUMPTION: The fix loop does not have a maximum iteration count; it continues until the user is satisfied -->

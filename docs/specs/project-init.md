...
Handles project initialization via `specshift init`, including template installation, constitution generation, agnostic bootstrap-file generation (`AGENTS.md` + `CLAUDE.md`), codebase scanning, and project health checks (spec drift, docs drift detection from the former docs-verify functionality).
...
- **Workflow template missing from plugin**: If the plugin's `workflow.md` template does not exist, report an error and suggest reinstalling the plugin.
- **Bootstrap template missing from plugin**: If the plugin's `agents.md` or `claude.md` bootstrap template does not exist, skip the corresponding bootstrap-file generation and warn that the plugin may need updating. Do not block init.

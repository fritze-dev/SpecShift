...
Provides all generated documentation: capability docs, Architecture Decision Records (ADRs), and consolidated README. Generated by `specshift finalize` (docs step) from specs and completed change artifacts.
...
- **AND** a spec exists at `docs/specs/user-auth.md`
- **WHEN** the developer runs `specshift finalize`
...
- **GIVEN** `.specshift/WORKFLOW.md` contains `docs_language: Spanish`
- **WHEN** the developer runs `specshift finalize`
...
- **THEN** the References section includes `[Change: improve-docs-quality](../../.specshift/changes/2026-03-05-improve-docs-quality/)` as the first reference
...
The `specshift finalize` command SHALL preserve manual ADRs during regeneration. Manual ADRs are files in `docs/decisions/` matching the pattern `adr-M*.md`. They use the `adr-MNNN-slug.md` naming convention (M prefix + 3-digit zero-padded number) to distinguish them from generated ADRs (`adr-NNN-slug.md`). The agent SHALL NOT delete, overwrite, or renumber manual ADR files. Manual ADRs SHALL be included in the `docs/README.md` Key Design Decisions table after all generated ADRs, with links like `[ADR-M001](decisions/adr-M001-init-model-invocable.md)`. The agent SHALL extract the Decision and Rationale from the manual ADR's `## Decision` and `## Rationale` sections for the table.
...
- **ADR not yet generated for a decision**: If ADR generation runs after overview generation within the same `specshift finalize` run, the agent SHALL generate ADRs first, then reference them in the overview. The step ordering ensures this.
...
- Specs in `docs/specs/` are the source of truth for documentation generation. <!-- ASSUMPTION: Docs generated from current specs -->
- Completed changes follow `YYYY-MM-DD-<feature-name>` naming under `.specshift/changes/`. <!-- ASSUMPTION: Naming enforced by new skill -->
...
- Doc templates at `.specshift/templates/docs/` are available in consumer projects after `specshift init` copies the templates. <!-- ASSUMPTION: Template copy includes subdirectories -->
...
- The `lastUpdated` frontmatter field in capability docs is only written by `specshift finalize`, not manually edited. <!-- ASSUMPTION: Users follow workflow conventions -->

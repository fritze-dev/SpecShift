...
Manages the change lifecycle including workspace creation (now part of `specshift propose`), date-prefixed workspace structure, worktree isolation, worktree context detection (now handled by the router), lazy worktree cleanup, and active/completed change detection based on task status.
...
- **Multiple changes in a worktree**: Each worktree should contain exactly one change matching the branch name. Additional `.specshift/changes/` directories are ignored by worktree detection.

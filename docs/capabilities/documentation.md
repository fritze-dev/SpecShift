...
Generates all user-facing documentation from specs and completed change artifacts via `specshift finalize`: enriched capability docs, Architecture Decision Records, and a consolidated README with architecture overview.
...
### README Generation (`specshift finalize`)

The README is built from the constitution (System Architecture, Tech Stack, Conventions), all ADR files (Key Design Decisions table with Notable Trade-offs), and all capability docs (grouped by spec `category` frontmatter, ordered by `order` field). Capability descriptions are concise (max 80 characters or 15 words). The README template at `.specshift/templates/docs/readme.md` defines the format.

...
- **WORKFLOW.md + Smart Templates Layer**: `.specshift/WORKFLOW.md` declares the 8-stage artifact pipeline order, inline action definitions (built-in and custom), apply gate, optional worktree configuration, and project context. Smart Templates in `.specshift/templates/` carry per-artifact instructions, output paths, and dependencies in YAML frontmatter. Together they serve as the single source of truth for pipeline structure, action instructions, and artifact generation.

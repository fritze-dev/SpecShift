...
For each unresolved review thread, the action reads the comment, determines if the feedback is actionable, implements the fix if so, replies to the thread, and resolves it. After all fixes are committed and pushed — and only after the external review decision (per `review.request_review`) is known — the action invokes a mandatory self-check on the current branch as the final verification step before the pre-merge summary. The canonical invocation form is to spawn a subagent whose prompt invokes the `review` skill on the current HEAD; the subagent boundary provides context isolation. Custom-prompted general-purpose subagents running a hand-written checklist do NOT satisfy this requirement — the prompt must invoke the `review` skill directly. Inline invocation via the Skill tool is permitted only as a fallback when subagent spawning is unavailable. The self-check posts a PR comment with `<!-- specshift:self-check -->` at the top, the HEAD commit SHA, and a PASS/FIX findings summary. If the self-check finds issues, they are fixed, committed, and the self-check is re-invoked until it reports PASS for the latest HEAD. If a reviewer posts new comments after fixes, the cycle repeats (up to 3 times).

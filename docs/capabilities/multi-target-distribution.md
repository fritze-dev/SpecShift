...
description: "Ship SpecShift to Claude Code and OpenAI Codex CLI from one repository with one shared skill body, an agnostic version source of truth, and per-target marketplace catalogs at the repo root."
lastUpdated: "2026-04-28"
---
# Multi-Target Distribution

SpecShift is packaged from a single repository for two AI-coding-tool targets — Claude Code and OpenAI Codex CLI — via per-target plugin manifests and marketplace catalogs at the repository root, one shared agnostic skill tree at `./skills/specshift/`, a bootstrap pattern that lets both runtimes read the same instructions without content duplication, and a single version source of truth at `src/VERSION` stamped symmetrically into the three version-bearing root files at compile time. The Codex marketplace catalog (`.agents/plugins/marketplace.json`) is the fourth root file; it is presence-and-shape-checked rather than version-stamped because the documented Codex catalog schema does not include a `plugins[].version` field.
...
- Hand-edited per-target marketplace catalogs at the repository root: `.claude-plugin/marketplace.json` (Claude) and `.agents/plugins/marketplace.json` (Codex, documented schema with object-form `source`, `policy`, `category`, no `plugins[].version`)
- Codex install via `codex plugin marketplace add fritze-dev/SpecShift`, then `/plugins`; the marketplace add resolves through `.agents/plugins/marketplace.json` to the generated Codex payload at `plugins/specshift/`
...
- Symmetric version stamping via `jq` into the three version-bearing root files with post-stamp cross-check; the Codex marketplace catalog is presence-and-shape-checked in the same pass; any drift fails the build
...
Codex consumers install via `codex plugin marketplace add fritze-dev/SpecShift`, then open `/plugins` and install or enable SpecShift. The marketplace add resolves through the catalog file `.agents/plugins/marketplace.json` at the repository root, which carries the documented Codex schema: top-level `name` + `interface.displayName`, single-entry `plugins[]` with object-form `source: { source: "local", path: "./plugins/specshift" }`, `policy: { installation: "AVAILABLE", authentication: "ON_INSTALL" }`, and `category: "Coding"`. The catalog has no `plugins[].version` field — version is sourced from `plugins/specshift/.codex-plugin/plugin.json`, which is regenerated from the stamped root `.codex-plugin/plugin.json`. An earlier iteration of this capability assumed `.codex-plugin/plugin.json` was auto-discovered without a catalog; that assumption was falsified by a user-observed install failure on the released `0.2.5-beta` layout (issue #51), and the catalog file plus generated Codex payload became mandatory in `0.2.6-beta`.
...
`src/VERSION` is the single agnostic version source of truth — plain text, single line, SemVer (e.g., `0.2.6-beta`). The compile script validates it against the SemVer 2.0 regex, then reads it once and stamps the value into the three version-bearing root files (`.claude-plugin/plugin.json` `.version`, `.claude-plugin/marketplace.json` `.plugins[].version`, `.codex-plugin/plugin.json` `.version`) via `jq`. After stamping, the script re-reads each file and verifies the stamped value matches `src/VERSION`; any mismatch fails the build with an error naming the offending file. The Codex marketplace catalog (`.agents/plugins/marketplace.json`) is presence-and-shape-checked in the same pass — the documented Codex schema does not carry a `plugins[].version` field, so the catalog is verified for top-level `name`, `interface.displayName`, single-entry `plugins[]` with object-form `source.path == "./plugins/specshift"`, `AVAILABLE`/`ON_INSTALL` policy, `Coding` category, and absence of `plugins[].version`. The compile script then regenerates `plugins/specshift/` from the stamped Codex manifest and compiled skill tree. The same verification is also enforced in CI (`.github/workflows/release.yml`) before tag creation, so a maintainer who edits `src/VERSION` without recompiling — or accidentally deletes/mangles the catalog or generated payload — is caught at push time. The compile script also stamps the version into the compiled workflow template's `plugin-version` frontmatter field. To bump the plugin version, the maintainer edits `src/VERSION` and runs `bash scripts/compile-skills.sh`.

### Multi-Target Install Documentation

The `README.md` carries one install section per supported target at the same heading level. Claude Code consumers run `claude plugin marketplace add fritze-dev/specshift` followed by `claude plugin install specshift`. Codex consumers run `codex plugin marketplace add fritze-dev/SpecShift`, then open `/plugins` and install or enable SpecShift. Update flows are target-specific and documented in their respective sections (Claude: `claude plugin marketplace update specshift && claude plugin update specshift@specshift`; Codex: `codex plugin marketplace upgrade specshift`).
...
- **Codex marketplace upstream schema change**: jq stamping preserves all non-version fields verbatim, so an additive schema change (new optional fields) Just Works on `.codex-plugin/plugin.json`. The catalog file (`.agents/plugins/marketplace.json`) is hand-edited; if the documented Codex schema evolves (renamed fields, new required fields), the catalog is updated directly. The compile script's shape verifier asserts the documented core fields; additive fields beyond the asserted set are preserved verbatim by leaving the file un-mutated by automation.
- **Catalog file accidentally deleted**: the compile script and release CI cross-check both fail with a maintainer-actionable error naming `.agents/plugins/marketplace.json`. No silent regression.

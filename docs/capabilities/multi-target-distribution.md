...
- Bootstrap single source of truth: full body in AGENTS.md; CLAUDE.md is opt-in (Claude Code consumers add a one-line `@AGENTS.md` stub manually if they want the documented memory pattern)
...
When `specshift init` runs in a fresh consumer project, it writes only `AGENTS.md` (full body, including Workflow, Planning, and Knowledge Management sections plus any project-specific content from the codebase scan). The plugin still ships `templates/claude.md` as a Smart Template containing the documented `@AGENTS.md` import line, but init does not auto-generate `CLAUDE.md`. Claude Code consumers who want the documented memory pattern create a hand-maintained `CLAUDE.md` themselves (typically by copying the stub). Codex reads AGENTS.md natively; Claude Code reads the user-maintained CLAUDE.md and expands the `@AGENTS.md` import to load AGENTS.md content into the session context. Updating workflow rules requires editing only the `agents.md` template — the change reaches both tools without duplication.
...
- If a consumer project initialized by a pre-multi-target plugin version still has only `CLAUDE.md`, re-running `specshift init` adds `AGENTS.md` (full body) without overwriting the existing CLAUDE.md, and reports that the user may want to collapse CLAUDE.md to a `@AGENTS.md` import stub manually.
- A consumer project that has only AGENTS.md after a fresh init remains as-is on re-init — no `CLAUDE.md` is auto-created.

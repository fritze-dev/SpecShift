...
The Codex marketplace at `.agents/plugins/marketplace.json` advertises the plugin to Codex consumers running `codex /plugins`. Its `plugins[].source` field references the Codex plugin manifest path (`./.codex-plugin`). The compile script stamps the current `src/VERSION` value into `plugins[].version` via `jq`, preserving every other field verbatim. The Codex marketplace is independent of the Claude marketplace — changes to one do not require changes to the other.

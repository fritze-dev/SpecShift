...
### Preflight: Quality Check During Propose (`specshift propose`)
...
### Docs Drift Detection: Health Check During Init (`specshift init`)
...
When a spec exists but no corresponding capability doc is found, the report includes a CRITICAL issue recommending `specshift finalize`.
...
- If no merge base is available (orphan branch, first commit), diff-based checks are skipped with a note. Finalize template-version validation is also skipped.
- If `docs/capabilities/` does not exist, all specs are reported as missing capability docs without erroring.
- If a modified template under `src/templates/` has no `template-version` field, finalize flags it as BLOCKED.
- If a template file is renamed or moved, finalize compares it to the base-branch file at the previous path, and `template-version` continues monotonically rather than resetting to `1`.

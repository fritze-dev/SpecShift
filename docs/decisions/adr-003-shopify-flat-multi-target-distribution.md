...
1. **Adopt the Shopify-flat layout.** Compiled skill tree moves to `./skills/specshift/` at the repo root. Both per-target manifests live hand-edited at the root: `.claude-plugin/plugin.json` and `.codex-plugin/plugin.json`. The Claude marketplace at `.claude-plugin/marketplace.json` declares `source: "./"`. The Codex marketplace at `.agents/plugins/marketplace.json` declares the Codex plugin entry at `source: "./.codex-plugin"`. The `src/.claude-plugin/` source-indirection layer is removed entirely — manifests are not built from source, they are authored at the root and only their `version` field is touched by automation.

2. **Introduce `src/VERSION` as the single agnostic version source of truth.** Plain text, single line, SemVer. The compile script reads it and stamps the value into all four root manifest/marketplace files (`.claude-plugin/plugin.json`, `.claude-plugin/marketplace.json`, `.codex-plugin/plugin.json`, `.agents/plugins/marketplace.json`) via `jq`, preserving every other key, ordering, and formatting verbatim. After stamping, the script re-reads each file and verifies the stamped value equals `src/VERSION`; any mismatch fails the build with an error naming the offending file. The `specshift finalize` version-bump step now edits only `src/VERSION`.
...
5. **`jq` becomes a hard build dependency.** The compile script preflights `command -v jq` and fails with a descriptive error if missing. Symmetric in-place version stamping that preserves field order and formatting requires `jq`; falling back to `sed`/`awk` would risk reformatting JSON.
...
- **Auto-derive `.agents/plugins/marketplace.json` `source` field from manifest location**: hand-edited keeps maintainer control without script complexity; Codex marketplace `source` semantics may evolve and a hand-edited field is one less moving part.
...
- **Symmetric versioning**: All four root manifest/marketplace files always agree on the plugin version; the compile-time cross-check makes drift impossible.

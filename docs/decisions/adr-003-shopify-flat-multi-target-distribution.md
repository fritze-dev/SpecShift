...
1. **Adopt the Shopify-flat layout.** Compiled skill tree moves to `./skills/specshift/` at the repo root. Both per-target manifests live hand-edited at the root: `.claude-plugin/plugin.json` and `.codex-plugin/plugin.json`. The Claude marketplace at `.claude-plugin/marketplace.json` declares `source: "./"`. Codex consumers install via `codex plugin marketplace add github:fritze-dev/specshift` (see Decision 6 for the resolved install surface). The `src/.claude-plugin/` source-indirection layer is removed entirely — manifests are not built from source, they are authored at the root and only their `version` field is touched by automation.
...
6. **Ship `.agents/plugins/marketplace.json` (Codex marketplace catalog).** *(Amendment 2026-04-28.)* The original decision relied on Codex CLI's documented single-plugin auto-discovery on `.codex-plugin/plugin.json`. After PR #46 shipped, a user reported that `codex plugin marketplace add github:fritze-dev/specshift` did not find the plugin — falsifying the auto-discovery assumption. The catalog file (previously listed as a rejected alternative) is now mandatory. The catalog ships at the repo root with the documented Codex schema: top-level `name` + `interface.displayName`; `plugins[]` with `name`, `description`, `source: { source: "local", path: "../../.codex-plugin" }` (object form, not bare string), `policy: { installation, authentication }` at documented baseline values, and `category`. The catalog SHALL NOT carry a `plugins[].version` field — version is sourced from `.codex-plugin/plugin.json` referenced by `plugins[0].source.path`. The compile script and release CI cross-check now cover four root files: the three version-bearing files keep their version-equality check; the catalog is presence-and-shape-checked. See [Issue #51](https://github.com/fritze-dev/SpecShift/issues/51) for the falsifying observation and the acceptance-criterion branch (b) the change implements.
...
- **Catalog `plugins[].source.path` requires manual upkeep**: if `.codex-plugin/`'s repo-root location ever changes, the catalog's `path` must be updated alongside it. The compile script's shape verifier asserts the path ends with `.codex-plugin`, which catches accidental drift.

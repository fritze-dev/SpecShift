...
2. **Introduce `src/VERSION` as the single agnostic version source of truth.** Plain text, single line, SemVer. The compile script reads it and stamps the value into all four root manifest/marketplace files (`.claude-plugin/plugin.json`, `.claude-plugin/marketplace.json`, `.codex-plugin/plugin.json`, `.agents/plugins/marketplace.json`) via `jq`, preserving all non-version fields and values semantically (JSON formatting may be normalized by `jq`'s pretty-printer; consumers depend on semantic content, not byte-level formatting). After stamping, the script re-reads each file and verifies the stamped value equals `src/VERSION`; any mismatch fails the build with an error naming the offending file. The `specshift finalize` version-bump step now edits only `src/VERSION`.

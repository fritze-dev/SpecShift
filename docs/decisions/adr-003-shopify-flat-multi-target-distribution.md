...
- **Live install verification on a real Codex installation deferred**: spec scenarios cover the install flow but execution against an actual Codex install was deferred when ADR-003 was first written (the original wording referenced the in-session `/plugins` UI as a stand-in). **Update (2026-04-28):** the auto-discovery assumption was falsified on a live Codex install, which led to Decision 6 (ship the catalog file). The verified install commands are `codex plugin marketplace add fritze-dev/SpecShift` followed by enabling SpecShift from the in-session `/plugins` directory; updates use `codex plugin marketplace upgrade specshift`. A clean-machine smoke test of the new catalog-mediated path is still pending (Issue #51 acceptance).

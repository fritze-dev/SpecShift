...
**Status:** Accepted (2026-04-27); amended 2026-04-28 with Decision 6 (ship Codex marketplace catalog).
...
1. **Adopt the Shopify-flat layout.** Compiled skill tree moves to `./skills/specshift/` at the repo root. Both per-target manifests live hand-edited at the root: `.claude-plugin/plugin.json` and `.codex-plugin/plugin.json`. The Claude marketplace at `.claude-plugin/marketplace.json` declares `source: "./"`. Codex consumers install via `codex plugin marketplace add fritze-dev/SpecShift` (see Decision 6 for the resolved install surface). The `src/.claude-plugin/` source-indirection layer is removed entirely — manifests are not built from source, they are authored at the root and only their `version` field is touched by automation.
...
6. **Ship `.agents/plugins/marketplace.json` plus a generated `plugins/specshift` Codex payload.** *(Amendment 2026-04-28.)* The original decision relied on Codex CLI's documented single-plugin auto-discovery on `.codex-plugin/plugin.json`. After PR #46 shipped, a user reported that `codex plugin marketplace add fritze-dev/SpecShift` did not surface the plugin — falsifying the auto-discovery assumption. Local verification on `codex-cli 0.125.0` also showed that root-path catalog entries are skipped by `/plugins`; Codex needs a non-empty plugin-root path under the marketplace root. The catalog file (previously listed as a rejected alternative) is now mandatory and points at `plugins/specshift/`. The catalog ships at the repo root with the documented Codex schema: top-level `name` + `interface.displayName`; `plugins[]` with `name`, `description`, `source: { source: "local", path: "./plugins/specshift" }` (object form, not bare string), `policy: { installation: "AVAILABLE", authentication: "ON_INSTALL" }`, and `category: "Coding"`. The catalog SHALL NOT carry a `plugins[].version` field — version is sourced from `plugins/specshift/.codex-plugin/plugin.json`, which is regenerated from the stamped root `.codex-plugin/plugin.json`. The compile script and release CI cross-check now cover the three version-bearing root files, the shape-only catalog, and the generated Codex payload. See [Issue #51](https://github.com/fritze-dev/SpecShift/issues/51) for the falsifying observation and the acceptance-criterion branch (b) the change implements.
...
- **Ship a `.agents/plugins/marketplace.json` catalog file**: this was originally rejected on the basis that the Codex documented behavior auto-discovers a single-plugin repo's `.codex-plugin/plugin.json` directly. **That decision was reversed on 2026-04-28 (see Decision 6 above).** A user-observed install failure on the released `0.2.5-beta` layout falsified the auto-discovery assumption, and local verification showed the Codex catalog must point at a non-empty plugin root such as `./plugins/specshift`. The catalog file is now shipped using the documented Codex schema (`name` + `interface.displayName`; `plugins[].source` as `{source: "local", path}` object; `plugins[].policy.installation`/`authentication`; no `plugins[].version`). The earlier concern that the schema is distinct from Claude's marketplace remains accurate — the catalog uses the Codex-documented form, and the compile script's shape verifier asserts that form.
...
- **Live install verification on a real Codex installation**: PR #46's release was verified post-merge by the user who reported the install failure that triggered Decision 6 — the failure observation IS the live verification of the original assumption. Re-verification of the new catalog-driven flow is a post-merge step on the `0.2.6-beta` release.
- **Generated Codex payload requires upkeep in the build**: if the Codex payload location ever changes, `.agents/plugins/marketplace.json`, `scripts/compile-skills.sh`, and release CI must be updated together. The compile script and CI assert `source.path == "./plugins/specshift"` and verify that the generated payload contains `.codex-plugin/plugin.json` and `skills/specshift/SKILL.md`.

...
The system SHALL provide `specshift init` as the single entry point for project setup. The init command SHALL: (1) copy pipeline Smart Templates from the plugin's `templates/` directory (at `the installed plugin root/templates/`) into the project's `.specshift/templates/` directory — excluding bootstrap templates (`workflow.md`, `constitution.md`, `agents.md`) which are used only to generate their target files, (2) generate `.specshift/WORKFLOW.md` from the plugin's workflow template at `the installed plugin root/templates/workflow.md` (skip if WORKFLOW.md already exists), (3) generate `.specshift/CONSTITUTION.md` from the plugin's constitution template if none exists, and (4) generate `AGENTS.md` from the bootstrap template at `the installed plugin root/templates/agents.md` if no AGENTS.md exists (see "AGENTS.md Bootstrap" requirement). The init command SHALL be idempotent — running it on an already-initialized project SHALL skip completed steps.
...
- **THEN** the system SHALL copy Smart Templates from `the installed plugin root/templates/` to `.specshift/templates/`, copy WORKFLOW.md from `the installed plugin root/templates/workflow.md`, create `.specshift/CONSTITUTION.md` placeholder, generate `AGENTS.md` from the bootstrap template, and verify the setup
...
- **AND** the plugin has `the installed plugin root/templates/workflow.md`
- **WHEN** the user runs `specshift init`
...
When `specshift init` runs on an already-initialized project (re-init after plugin update), the system SHALL use Smart Template `template-version` fields to detect user customizations and merge plugin updates instead of blindly overwriting. For each template file in `the installed plugin root/templates/`:
...
For AGENTS.md, the merge operates at **section level with WARNING-only reporting**: the system SHALL compare the bootstrap template's section headings (e.g., `## Workflow`, `## Knowledge Management`) against the existing AGENTS.md. Missing standard sections SHALL be reported as WARNING (e.g., "AGENTS.md missing standard section: Knowledge Management"). The system SHALL NOT modify the existing AGENTS.md — user edits are authoritative. This check ensures that standard agent directives are not silently lost when AGENTS.md is manually edited or created before the plugin is installed.

...
A user reported that `codex plugin marketplace add fritze-dev/SpecShift` did not surface the plugin on the released `0.2.5-beta` layout, falsifying the Codex single-plugin auto-discovery assumption that drove PR #46. This release ships the documented Codex marketplace catalog file at `.agents/plugins/marketplace.json` plus a generated Codex payload at `plugins/specshift/`, so Codex consumers can add the marketplace and then install or enable SpecShift from `/plugins`. Issue #51 acceptance branch (b).

#### Added
- `.agents/plugins/marketplace.json` (hand-edited at the repo root) following the documented Codex schema verified against `codex-cli 0.125.0`: top-level `name` + `interface.displayName`, single-entry `plugins[]` with object-form `source: { source: "local", path: "./plugins/specshift" }`, `policy: { installation: "AVAILABLE", authentication: "ON_INSTALL" }`, `category: "Coding"`. The catalog has no `plugins[].version` field — version is sourced from `plugins/specshift/.codex-plugin/plugin.json`
- `plugins/specshift/` generated Codex marketplace payload containing `.codex-plugin/plugin.json` and the compiled `skills/specshift/` tree
...
#### Changed
- README Codex install section: replaced the in-session `codex /plugins` line with `codex plugin marketplace add fritze-dev/SpecShift` plus a `/plugins` install/enable instruction; added a Codex Update subsection using `codex plugin marketplace upgrade specshift`; layout diagram now shows `.agents/plugins/` and `plugins/specshift/`; version-stamping paragraph differentiates the three version-bearing files from the shape-checked catalog and generated Codex payload (Closes #51)

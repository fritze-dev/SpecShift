...
- **`docs/specs/quality-gates.md` v3 → v4:** Audit Testing dimension reworded to reference "specs (direct scenario verification)" plus apply-phase tests; four test-coverage scenarios rewritten as scenario-verification-against-specs; "Preflight Side-Effect Cross-Check" renamed to "Pre-Implementation Side-Effect Cross-Check" while keeping preflight.md as the source; positional refs `tasks.md step 3.2` / `step 3.5` replaced with semantic anchors (`tasks.md § Standard Tasks (Post-Implementation)`, "post-implementation QA loop").
...
- **`src/templates/changes/tasks.md` template-version 5 → 6:** `requires: [preflight]`; apply-phase test guidance added (framework configured → automated tests; "None" → audit-driven scenario verification).
- **`src/templates/changes/audit.md` template-version 3 → 4:** tests references → "specs (direct scenario verification)"; Validation Side-Effects dimension reads from preflight.md.

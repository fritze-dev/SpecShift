...
- New Codex marketplace entry hand-edited at `.agents/plugins/marketplace.json` (root) so the plugin is discoverable via `codex /plugins`; compile script stamps the version via `jq`
...
Second-pass extension on top of the first commit, aligning the implementation with the original goal: one agnostic skill body served to both Claude Code and Codex from a single compiled tree, hand-edited per-target manifests at the repo root. Fresh init generates both `AGENTS.md` (full body, agnostic source of truth) and `CLAUDE.md` (one-line `@AGENTS.md` import stub) so Claude Code's documented memory-import pattern is active without manual setup — the stub is a pointer, not a content duplicate, so SSOT is preserved.

#### Changed
- **BREAKING (manifest + marketplace source location):** Plugin manifests AND marketplace files are now hand-edited at the repo root (`.claude-plugin/plugin.json`, `.claude-plugin/marketplace.json`, `.codex-plugin/plugin.json`, `.agents/plugins/marketplace.json`) instead of under `src/.claude-plugin/`, `src/.codex-plugin/`, or `src/marketplace/`. The `src/` manifest and marketplace directories are deleted. The compile script reads the version from `.claude-plugin/plugin.json` (the source of truth) and stamps it into both the Codex manifest and the Codex marketplace in place via `jq`, preserving all other fields verbatim.
...
- `src/templates/workflow.md` v9 → 11: `## Action: init` rewritten — fresh init generates both AGENTS.md (full body) and CLAUDE.md (one-line `@AGENTS.md` import stub); existing files are never overwritten on re-init; standard-sections checks remain passive WARNING-only. Commented worktree `path_pattern` default → `.specshift/worktrees/{change}`. `.specshift/WORKFLOW.md` synced.

#### Specs
- `release-workflow.md` v3 → 5: Auto Patch Version Bump, Version Sync Between Plugin Files, Manual Minor and Major Release Process, Source and Release Directory Structure, Marketplace Source Configuration, Repository Layout Separation, AOT Skill Compilation, Compiled Action File Contract, Dev Sync Script — all rewritten for multi-target reality with manifests AND marketplace files hand-edited at the repo root and `jq`-based version stamping
- `multi-target-distribution.md` v1 → 4: Per-Target Plugin Manifest revised for hand-edited root manifests with enrichment; Codex Marketplace Entry revised for hand-edited root location (no more `src/marketplace/codex.json`); Bootstrap SSOT revised — fresh init generates both AGENTS.md and the CLAUDE.md `@AGENTS.md` import stub; new **Agnostic Skill Body** requirement
- `project-init.md` v6 → 8: `${CLAUDE_PLUGIN_ROOT}` references replaced with prose; **Bootstrap Files Generation** requirement: fresh init writes both `AGENTS.md` (full body) and `CLAUDE.md` (one-line `@AGENTS.md` import stub); existing files are never overwritten on re-init

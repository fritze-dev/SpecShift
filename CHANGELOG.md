...
- `docs/specs/release-workflow.md` v6: Requirement "Source and Release Directory Structure" — "no separate Codex marketplace catalog file is shipped" sentence replaced with the four-file description (three version-bearing + one catalog without `version` field)
...
- `README.md` Installation → OpenAI Codex CLI: replaced `codex /plugins` placeholder with canonical `codex plugin marketplace add github:fritze-dev/specshift` + `codex plugin install specshift` plus an Update subsection
